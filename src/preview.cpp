...
void Preview::run(sc::PreviewReplyProxy const& reply) {
    ...
    layout1col.add_column( { "image_widget", "header_widget", "summary_widget", "actions" } );

    // Two column layout
    layout2col.add_column( { "image_widget" } );
    layout2col.add_column( { "header_widget", "summary_widget", "actions" } );

    // Three cokumn layout
    layout3col.add_column( { "image_widget" });
    layout3col.add_column( { "header_widget", "summary_widget", "actions" } );
    ...
    header.add_attribute_mapping("summary", "summary");
  ...
  /*  builder.add_tuple({
        {"id", sc::Variant("open")},
        {"label", sc::Variant(" ▻ YouTube")},
        {"uri", result["yurl"]}
    }); */
 /*   builder.add_tuple({
        {"id", sc::Variant("open")},
        {"label", sc::Variant(" ♡ Like")},
        {"uri", result["re"]}
    });*/
